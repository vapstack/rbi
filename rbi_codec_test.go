...
func (r *codecNamedTimeRec) EncodeRBI(w io.Writer) error {
	return binary.Write(w, binary.BigEndian, time.Time(r.When).Unix())
}

func (r *codecNamedTimeRec) DecodeRBI(reader io.Reader) error {
	var sec int64
	if err := binary.Read(reader, binary.BigEndian, &sec); err != nil {
		return err
	}
	r.When = codecNamedTime(time.Unix(sec, 0).UTC())
	return nil
}
...
func TestCodec_SetOnChangeSnapshotUsesCodecMethods(t *testing.T) {
...
func TestCodec_NewRejectsValueReceiverDecodeRBI(t *testing.T) {
	...
	_, err := Open[uint64, codecBadValueDecodeRec](raw, testOptions(Options{}))
	if err == nil {
...
func TestCodec_AllowsNamedTimeIndexField(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "named_time_codec.db")
	c, bolt := openBoltAndCollection[uint64, codecNamedTimeRec](t, path, Options{})
	defer func() {
		_ = c.Close()
		_ = bolt.Close()
	}()
	...
	if err := writeSet(c, 1, &codecNamedTimeRec{When: codecNamedTime(base)}); err != nil {
		t.Fatalf("Set(1): %v", err)
	}
	if err := writeSet(c, 2, &codecNamedTimeRec{When: codecNamedTime(later)}); err != nil {
	...
	defer c.ReleaseRecords(got)
	if time.Time(got.When) != base {
		t.Fatalf("Get(1) When=%v want %v", time.Time(got.When), base)
	}

	keys, err := readQueryKeys(c, qx.Query(qx.EQ("when", base)))
	if err != nil {
		t.Fatalf("QueryKeys(EQ when): %v", err)
	}
	if !slices.Equal(keys, []uint64{1}) {
		t.Fatalf("QueryKeys(EQ when)=%v want [1]", keys)
	}
}
...
func TestCodec_RecordPool_ReusedDecodeLeaksAbsentFields(t *testing.T) {
	c, _ := openTempCollectionUint64CodecSparseRec(t, Options{BatchSoftLimit: 1})

	opt := "sticky"
	if err := writeSet(c, 1, &codecSparseRec{
		Name: "first",
		Tags: []string{"go", "db"},
		Opt:  &opt,
	}); err != nil {
	...
	if err := writeSet(c, 2, &codecSparseRec{Name: "second"}); err != nil {
		t.Fatalf("Set(2): %v", err)
	}
...
func openTempCollectionUint64PooledSparseRec(t *testing.T, options ...Options) (*Collection[uint64, pooledSparseRec], string) {
	t.Helper()
	dir := t.TempDir()
	path := filepath.Join(dir, "test_pooled_sparse.db")
	c, bolt := openBoltAndCollection[uint64, pooledSparseRec](t, path, options...)
	t.Cleanup(func() {
		_ = c.Close()
		_ = bolt.Close()
	})
...
func TestRecordPool_ReusedDecodeLeaksAbsentFields(t *testing.T) {
	c, _ := openTempCollectionUint64PooledSparseRec(t, Options{BatchSoftLimit: 1})

	opt := "sticky"
	if err := writeSet(c, 1, &pooledSparseRec{
		Name: "first",
		Tags: []string{"go", "db"},
		Opt:  &opt,
	}); err != nil {
	...
	if err := writeSet(c, 2, &pooledSparseRec{Name: "second"}); err != nil {
		t.Fatalf("Set(2): %v", err)
	}
...
func TestRecordPoolReleaseZeroesMsgpackRecord(t *testing.T) {
	c, _ := openTempCollectionUint64PooledSparseRec(t)
