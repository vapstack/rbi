...
type poolSample struct {
	...
	Stats      rbi.PoolStats `json:"stats"`
...
func resetPoolStatsWindow(db *rbi.DB[uint64, UserBench]) {
	db.ResetPoolStats()
}

func poolStatsSinceReset(db *rbi.DB[uint64, UserBench]) rbi.PoolStats {
	return db.PoolStatsSinceReset()
}
...
func poolReportData(poolBase, poolFinal *poolSample, poolSamples []poolSample) (*poolSample, *poolSample, *poolSummary, []poolSample) {
	return poolBase, poolFinal, summarizePools(poolSamples, poolFinal), poolSamples
}
