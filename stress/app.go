...
func makeBatchSample(now time.Time, baseline, current rbi.AutoBatchStats) batchSample {
	return batchSample{
		...
		Delta: batchDelta{
			...
			FallbackDisabled:    deltaUint64(current.FallbackDisabled, baseline.FallbackDisabled),
			FallbackQueueFull:   deltaUint64(current.FallbackQueueFull, baseline.FallbackQueueFull),
			FallbackPatchUnique: deltaUint64(current.FallbackPatchUnique, baseline.FallbackPatchUnique),
