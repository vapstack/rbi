...
func TestStringWritesRejectEmptyKey(t *testing.T) {
	...
	if err := writeSet(c, "", rec, OnChange(func(*Tx, string, *Rec, *Rec) error {
		onChangeCalled = true
		return nil
	...
	if onChangeCalled {
		t.Fatalf("OnChange ran for empty Set key")
	}

	if err := writeSets(c, []string{"ok", ""}, []*Rec{{Name: "ok"}, rec}); !errors.Is(err, berrors.ErrKeyRequired) {
		t.Fatalf("MultiSet empty key error = %v, want ErrKeyRequired", err)
	}
	if err := writePatch(c, "", []Field{{Name: "name", Value: "x"}}); !errors.Is(err, berrors.ErrKeyRequired) {
		t.Fatalf("Patch empty key error = %v, want ErrKeyRequired", err)
	}
	if err := writePatches(c, []string{"ok", ""}, []Field{{Name: "name", Value: "x"}}); !errors.Is(err, berrors.ErrKeyRequired) {
		t.Fatalf("MultiPatch empty key error = %v, want ErrKeyRequired", err)
	}
	if err := writeDelete(c, ""); !errors.Is(err, berrors.ErrKeyRequired) {
		t.Fatalf("Delete empty key error = %v, want ErrKeyRequired", err)
	}
	if err := writeDeletes(c, []string{"ok", ""}); !errors.Is(err, berrors.ErrKeyRequired) {
		t.Fatalf("MultiDelete empty key error = %v, want ErrKeyRequired", err)
	}
}
...
func TestCloseWaitsForSubmittedWrite(t *testing.T) {
	...
	go func() {
		setDone <- writeSet(c, 1, &Rec{Name: "one"}, OnChange(func(*Tx, uint64, *Rec, *Rec) error {
			close(entered)
			<-release
			return nil
		}))
	...
	case err := <-closeDone:
		timer.Stop()
		close(release)
		t.Fatalf("Close returned before submitted write finished: %v", err)
	case <-timer.C:
	}
...
func TestIOExt_Patch_CorruptPayloadReturnsErrorAndLeavesOtherRowsReadable(t *testing.T) {
	c, _ := openTempUint64Collection(t, Options{BatchSoftLimit: 1})
	ioExtMustSetRec(t, c, 1, &Rec{Name: "one", Age: 10})
	ioExtMustSetRec(t, c, 2, &Rec{Name: "two", Age: 20})
	ioExtMustCorruptUint64Raw(t, c, 2, []byte{0xff, 0x00, 0x7f})

	err := writePatch(c, 2, []Field{{Name: "age", Value: 99}})
...
func TestIOExt_MultiDelete_DuplicateIDs_CallbackRunsOncePerExistingRecord(t *testing.T) {
	c, _ := openTempUint64Collection(t, Options{BatchSoftLimit: 1})
	ioExtMustSetRec(t, c, 1, &Rec{Name: "one"})
	ioExtMustSetRec(t, c, 2, &Rec{Name: "two"})

	var calls []uint64
	err := writeDeletes(c, []uint64{1, 1, 2, 2}, OnChange(func(_ *Tx, key uint64, oldValue, newValue *Rec) error {
...
func TestIOExt_MultiSet_DuplicateIDs_FinalValueMatchesLastWriteAcrossReadPaths(t *testing.T) {
	c, _ := openTempUint64Collection(t, Options{BatchSoftLimit: 1})
	...
	vals, err := readValues(c, 1, 2)
	if err != nil {
		t.Fatalf("readValues: %v", err)
	}
	if vals[0] == nil || vals[0].Name != "second" || vals[1] == nil || vals[1].Name != "other" {
		t.Fatalf("unexpected readValues state: %#v", vals)
	}

	scanned := ioExtCollectSeqScanRec(t, c, 0)
	if scanned[1].Name != "second" || scanned[2].Name != "other" {
		t.Fatalf("unexpected SeqScan state: %#v", scanned)
	}

	if ids := ioExtMustQueryName(t, c, "first"); len(ids) != 0 {
		t.Fatalf("stale name index for overwritten value: %v", ids)
	}
	if ids := ioExtMustQueryName(t, c, "second"); !slices.Equal(ids, []uint64{1}) {
		t.Fatalf("unexpected name index for second: %v", ids)
	}
}

func TestIOExt_MultiPatch_DuplicateIDs_FinalValueMatchesLastPatchAcrossReadPaths(t *testing.T) {
	c, _ := openTempUint64Collection(t, Options{BatchSoftLimit: 1})
	ioExtMustSetRec(t, c, 1, &Rec{Name: "seed-1", Age: 10, Meta: Meta{Country: "NL"}})
	ioExtMustSetRec(t, c, 2, &Rec{Name: "seed-2", Age: 20, Meta: Meta{Country: "DE"}})
	...
	if ids := ioExtMustQueryName(t, c, "step-1"); len(ids) != 0 {
		t.Fatalf("stale name index for intermediate patch value: %v", ids)
	}
}

func TestMultiSet_DuplicateIDs_LastWriteWinsAndIndexConsistent(t *testing.T) {
	c, _ := openTempUint64Collection(t, Options{BatchSoftLimit: 1})

	if err := writeSet(c, 1, &Rec{Name: "seed", Age: 21, Tags: []string{"go"}, Meta: Meta{Country: "NL"}}); err != nil {
	...
	if err := writeSets(c, []uint64{1, 1}, []*Rec{valA, valB}); err != nil {
...
func TestMultiSet_NilValue_ReturnsErrNilValueAndAtomic(t *testing.T) {
	...
	if err := writeSet(c, 1, base); err != nil {
	...
	if err == nil || !errors.Is(err, rbierrors.ErrNilValue) {
		t.Fatalf("expected rbierrors.ErrNilValue from MultiSet, got: %v", err)
	}
	...
	if v1 == nil || v1.Name != "base" || v1.Age != 21 || v1.Country != "NL" {
		t.Fatalf("id=1 changed after rejected MultiSet: %#v", v1)
	}
	...
	ids, err := readQueryKeys(c, qx.Query(qx.EQ("name", "changed")))
	if err != nil {
	...
	if cnt, err := readCount(c); err != nil {
		...
		t.Fatalf("expected Count=1 after rejected MultiSet, got %d", cnt)
	}

	assertNoFutureSnapshotRefs(t, c)
}

func TestMultiPatchMultiDelete_DuplicateIDs_IndexConsistency(t *testing.T) {
	c, _ := openTempUint64Collection(t, Options{BatchSoftLimit: 1})

	if err := writeSet(c, 1, &Rec{Name: "n1", Age: 10, Tags: []string{"go"}, Meta: Meta{Country: "NL"}}); err != nil {
		t.Fatalf("Set(1): %v", err)
	}
	if err := writeSet(c, 2, &Rec{Name: "n2", Age: 20, Tags: []string{"db"}, Meta: Meta{Country: "DE"}}); err != nil {
	...
	ids, err := readQueryKeys(c, qx.Query(qx.HASANY("tags", []string{"go"})))
	if err != nil {
	...
	ids, err = readQueryKeys(c, qx.Query(qx.HASANY("tags", []string{"db"})))
	if err != nil {
	...
	ids, err = readQueryKeys(c, qx.Query(qx.HASANY("tags", []string{"ops"})))
	if err != nil {
	...
	if err := writeDeletes(c, []uint64{1, 1, 2, 2}); err != nil {
		t.Fatalf("MultiDelete duplicate ids: %v", err)
	}

	if cnt, err := readCount(c); err != nil {
		t.Fatalf("Count: %v", err)
	} else if cnt != 0 {
		t.Fatalf("expected Count=0 after duplicate MultiDelete, got %d", cnt)
	}
...
func TestMultiPatch_DecodeError_RollsBackEarlierWrites(t *testing.T) {
	c, _ := openTempUint64Collection(t, Options{BatchSoftLimit: 1})

	if err := writeSet(c, 1, &Rec{Name: "a", Age: 10, Meta: Meta{Country: "NL"}}); err != nil {
		t.Fatalf("Set(1): %v", err)
	}
	if err := writeSet(c, 2, &Rec{Name: "b", Age: 20, Meta: Meta{Country: "DE"}}); err != nil {
		t.Fatalf("Set(2): %v", err)
	}
	...
	if err := c.root.bolt.View(func(tx *bbolt.Tx) error {
		...
		var keyBuf [8]byte
		v := b.Get(keycodec.UserKeyBytesWithBuf(uint64(1), c.strKey, &keyBuf))
		if v == nil {
	...
	if err := c.root.bolt.Update(func(tx *bbolt.Tx) error {
		...
		var keyBuf [8]byte
		return b.Put(keycodec.UserKeyBytesWithBuf(uint64(2), c.strKey, &keyBuf), []byte{0xff, 0x00, 0x7f, 0x42})
	}); err != nil {
		t.Fatalf("corrupt id=2 payload: %v", err)
	}

	err := writePatches(c, []uint64{1, 2}, []Field{{Name: "age", Value: 99}})
	if err == nil {
		t.Fatalf("expected MultiPatch decode error, got nil")
	}
	if !strings.Contains(err.Error(), "failed to decode existing value") {
		t.Fatalf("unexpected MultiPatch error: %v", err)
	}
	...
	if v1 == nil || v1.Age != 10 || v1.Name != "a" || v1.Country != "NL" {
		t.Fatalf("id=1 changed after failed MultiPatch: %#v", v1)
	}
}

func TestMultiDelete_DecodeError_RollsBackEarlierDeletes(t *testing.T) {
	c, _ := openTempUint64Collection(t, Options{BatchSoftLimit: 1})
	...
	err := writeDeletes(c, []uint64{1, 2})
	if err == nil {
		t.Fatalf("expected MultiDelete decode error, got nil")
	}
	if !strings.Contains(err.Error(), "decode") {
		t.Fatalf("unexpected MultiDelete error: %v", err)
	}
	...
	if v1 == nil || v1.Name != "a" || v1.Age != 10 || v1.Country != "NL" {
		t.Fatalf("id=1 changed after failed MultiDelete: %#v", v1)
	}
}
...
func TestMultiWrite_CallbackError_RollbackDataAndIndex(t *testing.T) {
	type tc struct {
		name string
		run  func(c *Collection[uint64, Rec], cb func(*Tx, uint64, *Rec, *Rec) error) error
	}
	...
	cases := []tc{
		...
		},
		{
			name: "batch_delete",
			run: func(c *Collection[uint64, Rec], cb func(*Tx, uint64, *Rec, *Rec) error) error {
				return writeDeletes(c, []uint64{1, 2}, OnChange(cb))
			},
		},
	}
	...
	for _, cs := range cases {
		...
		t.Run(cs.name, func(t *testing.T) {
			...
			for id, v := range base {
				cp := *v
				if err := writeSet(c, id, &cp); err != nil {
					t.Fatalf("seed Set(%d): %v", id, err)
				}
			}

			cbCalls := 0
			cb := func(_ *Tx, _ uint64, _ *Rec, _ *Rec) error {
				cbCalls++
...
func TestMultiWrite_CallbackError_RandomizedAtomicRollback(t *testing.T) {
	...
	for i := 1; i <= 70; i++ {
		id := uint64(i)
		if err := writeSet(c, id, randomRec(id)); err != nil {
			t.Fatalf("seed Set(%d): %v", id, err)
		}
	}
	...
	for step := 0; step < 80; step++ {
		...
		var opErr error
		switch step % 3 {
		case 0: // MultiSet with potential duplicate ids
			n := 3 + r.IntN(6)
			ids := make([]uint64, n)
			vals := make([]*Rec, n)
			for i := 0; i < n; i++ {
			...
			opErr = writeSets(c, ids, vals, OnChange(cb))
		case 1: // MultiPatch
			n := 3 + r.IntN(6)
			ids := make([]uint64, n)
			for i := 0; i < n; i++ {
			...
			opErr = writePatches(c, ids, patch, OnChange(cb))
		default: // MultiDelete
			n := 2 + r.IntN(6)
			ids := make([]uint64, n)
			for i := 0; i < n; i++ {
			...
			opErr = writeDeletes(c, ids, OnChange(cb))
		}
...
func TestMultiPatch_MissingIDs_CallbacksOnlyForExisting(t *testing.T) {
	...
	var called []string
	cb := func(_ *Tx, key string, oldValue, newValue *Product) error {
		called = append(called, key)
		return nil
	}

	err := writePatches(c, []string{"missing", "p1", "missing2"}, []Field{{Name: "price", Value: 20.0}}, OnChange(cb))
	if err != nil {
		t.Fatalf("MultiPatch: %v", err)
	}
